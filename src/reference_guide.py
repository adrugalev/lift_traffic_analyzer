...
PROFILE_EXAMPLES = {
    "gost": (
        "Нормативная проверка лифтовой группы: для жилого здания принимается не "
        "менее 6% населения за пять минут, для офиса или гостиницы — не менее 12%, "
        "при этом весь расчётный поток считается входящим."
    ),
    "up_peak": (
